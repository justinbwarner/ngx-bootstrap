...
var BsDatepickerInputDirective = (function () {
    ...
    BsDatepickerInputDirective.prototype.doFormat = function (target) {
        var format = "mm/dd/yyyy";
        var match = new RegExp(format
            .replace(/(\w+)\W(\w+)\W(\w+)/, "^\\s*($1)\\W*($2)?\\W*($3)?([0-9]*).*")
            .replace(/m|d|y/g, "\\d"));
        var replace = "$1/$2/$3$4"
            .replace(/\//g, function () { return format.match(/\W/); });
    ...
    BsDatepickerInputDirective.prototype.onKeyup = function (event) {
